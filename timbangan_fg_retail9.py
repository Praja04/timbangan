    ...
    def _save_data(self, source_override=None):
        ...
        if not ok:
            ...
            lanjut = messagebox.askyesno(
                ...
                f"  ▸ YA   → dikirim dengan status NOT OK\n"
                f"  ▸ TIDAK → batal",
                icon="warning")
            if not lanjut:
                return
        ...
        form   = {
            "nik": nik, "mesin": self.sel_machine,
            "variant": self.sel_variant, "waktu": source["timestamp"],
            "berat": str(w), "unit": source["unit"],
            "status": status, "filler": filler,
        }

        # Simpan snapshot semua state yang dibutuhkan SEKARANG
        # agar tidak terpengaruh perubahan state di thread utama
        snapshot = {
            "sel_variant": self.sel_variant,
            "sel_machine": self.sel_machine,
        }
        ...
        def _post_with_retry():
            for attempt in range(1, MAX_RETRY + 1):
                self.root.after(0, self._update_loading,
                                f"Mengirim data... (percobaan {attempt}/{MAX_RETRY})")
                try:
                    ...
                    try:
                        ...
                        # Server return bukan JSON (HTML error page, dsb.)
                        # Ini pasti gagal — JANGAN anggap sukses
                        result["api_status"] = "Error Server"
                        result["error"] = (
                            f"Server mengembalikan non-JSON "
                            f"(HTTP {resp.status_code}). "
                            f"Preview: {resp.text[:120]}"
                        )
                        # Tidak ada gunanya retry jika server error parah
                        if resp.status_code < 500:
                            break
                        # Server 5xx → boleh retry
                        if attempt < MAX_RETRY:
                            time.sleep(RETRY_DELAY)
                        continue

                    # ── Controller Laravel return 201 + success:true ───
                    if resp.status_code == 201 and json_data.get("success") is True:
                        result["ok"]         = True
                        result["api_status"] = "✓ Terkirim"
                        break  # SUKSES — keluar dari loop

                    # ── HTTP 422: Validasi Laravel gagal ──────────────
                    if resp.status_code == 422:
                        errors     = json_data.get("errors", {})
                        err_detail = "; ".join(
                            f"{k}: {v[0] if isinstance(v, list) else v}"
                            for k, v in errors.items()
                        ) if errors else json_data.get("message", "Validation error")
                        result["api_status"] = "Validasi Gagal"
                        result["error"]      = f"Server menolak data: {err_detail}"
                        break  # 422 tidak perlu retry — pasti gagal lagi

                    # ── Response lain yang tidak dikenal ──────────────
                    result["api_status"] = f"HTTP {resp.status_code}"
                    result["error"]      = (
                        f"Response tidak dikenal: "
                        f"success={json_data.get('success')}, "
                        f"status={resp.status_code}"
                    )
                    if resp.status_code < 500:
                        break  # 4xx → tidak perlu retry
                    ...
                    result["error"]      = "Server tidak merespons dalam 8 detik"
                except Exception as e:
                ...
                if attempt < MAX_RETRY:
                    self.root.after(0, self._update_loading,
                                    f"Gagal, mencoba ulang ({attempt}/{MAX_RETRY - 1})...")
                    time.sleep(RETRY_DELAY)

            # Serahkan hasil ke main thread
            self.root.after(0, self._on_post_done, result, source, form, snapshot)

        self._show_loading("Menghubungi server...")
        threading.Thread(target=_post_with_retry, daemon=True).start()

    
    def _on_post_done(self, result, source, form, snapshot):
        """Dipanggil dari main thread setelah POST selesai."""
        self._hide_loading()

        if not result["ok"]:
            # Reset frozen data agar SPACE bisa dipakai ulang
            self._frozen_data     = None
            self._confirm_pending = False
        ...
        # Gunakan snapshot state (bukan self.sel_* yang mungkin sudah berubah)
        record = {
            **source,
            "nik":        form["nik"],
            "machine":    snapshot["sel_machine"],
            "variant":    snapshot["sel_variant"],
            "status":     form["status"],
            "filler":     form["filler"],
