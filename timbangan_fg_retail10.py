...
Revisi v10 (OFFLINE / EXPORT ONLY):
  - TIDAK mengirim data ke API / server
  - Data hanya disimpan di memori PC selama sesi berjalan
  - Hanya bisa di-export ke file Excel (.xlsx)
...
# ── FILTER KERAS — hanya USB-SERIAL CH340 ──────────────────
...
class ScaleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Timbangan AND GX-4000 — QC (Offline / Export Only)")
    ...
    def _build_header(self):
        ...
        tk.Label(tf, text="Quality Control · Offline / Export Only", font=F_SUB, bg=RED, fg=WHITE).pack(anchor="w")

        # ── Mode badge: OFFLINE ──
        mode_badge = tk.Frame(hdr, bg="#FEF3C7", highlightbackground="#FDE68A", highlightthickness=1)
        mode_badge.grid(row=0, column=2, sticky="e", padx=(0, 6), pady=18)
        tk.Label(mode_badge, text="📁 OFFLINE", font=("Segoe UI", 8, "bold"),
                 bg="#FEF3C7", fg="#92400E", padx=8, pady=2).pack()

        badge = tk.Frame(hdr, bg=RED, highlightbackground=WHITE, highlightthickness=1)
        badge.grid(row=0, column=3, sticky="e", padx=(0, 8), pady=15)
        ...
        tk.Button(hdr, text="↺ Reconnect",
            ...
            row=0, column=4, sticky="e", padx=12, pady=15)
    ...
    def _build_table(self, parent, row):
        ...
        # v10: kolom "API" dihapus — tidak ada pengiriman ke server
    ...
    def _build_footer(self):
        ...
        self.footer_lbl = tk.Label(
            ft, text="Port: Mendeteksi CH340... · Mode: OFFLINE (Export Only)",
    ...
    def _set_conn_status(self, connected, detail):
        if connected:
            ...
            self.footer_lbl.config(text=f"Port: {detail} (auto) · 9600bps · Mode: OFFLINE")
        else:
            self.conn_dot.config(fg=GRAY_400)
            self.conn_lbl.config(text=detail)
            self.footer_lbl.config(text=f"Port: {detail} · Mode: OFFLINE")
    ...
    # ─────────────────────────────────────────────────────────────
    # SAVE DATA — v10: LANGSUNG SIMPAN LOKAL, TANPA API
    # ─────────────────────────────────────────────────────────────
    def _save_data(self, source_override=None):
        ...
        if not ok:
            selisih = (std["min"] - w) if w < std["min"] else (w - std["max"])
            arah    = "BAWAH minimum" if w < std["min"] else "ATAS maksimum"
            batas   = std["min"] if w < std["min"] else std["max"]
            keterangan = (
                f"Berat terbaca : {w:.2f} g\n"
                f"Batas standar : {batas:.2f} g\n"
                f"Selisih       : {selisih:.2f} g\n\n"
                f"Berat DI {arah} standar."
            )
            lanjut = messagebox.askyesno(
                "⚠ Berat NOT OK — Konfirmasi Penyimpanan",
                f"Data ini berstatus NOT OK!\n\n{keterangan}\n"
                f"Tetap simpan data NOT OK ini?\n\n"
                f"  ▸ YA   → disimpan dengan status NOT OK\n"
                f"  ▸ TIDAK → batal",
                icon="warning")
            if not lanjut:
                return
        ...
        # v10: Langsung simpan ke memori lokal — TANPA kirim ke API
        record = {
            **source,
            "nik":      nik,
    ...
    # ─────────────────────────────────────────────────────────────
    # EXPORT EXCEL — v10: tanpa kolom API
    # ─────────────────────────────────────────────────────────────
    def _export_excel(self):
        ...
        try:
            ...
            c = ws["A1"]
            c.value = "DATA TIMBANGAN — AND GX-4000 | Quality Control (Offline / Export Only)"
            c.font  = Font(name="Calibri", size=13, bold=True, color="E53E3E")
            c.fill  = PatternFill("solid", fgColor="FFF5F5")
            c.alignment = Alignment(horizontal="center", vertical="center")
            ws.row_dimensions[1].height = 28
            # v10: kolom "API" dihapus
            headers = ["No","NIK","Mesin","Variant","Filler","Tanggal","Waktu","Berat","Unit","Status"]
            ...
            for ci, w in enumerate([6,12,10,24,8,14,10,10,8,10], 1):
            ...
            ws.cell(sr+4, 1, f"Mode   : OFFLINE (Export Only)")
            wb.save(fn)
            messagebox.showinfo(
                "Export Berhasil",
    ...
    def _on_close(self):
        if self.saved_data:
            msg = (f"Apakah Anda yakin ingin menutup aplikasi?\n\n"
                   f"⚠ Ada {len(self.saved_data)} data yang belum di-export!\n"
                   f"Pastikan sudah export ke Excel sebelum menutup.")
        else:
            msg = "Apakah Anda yakin ingin menutup aplikasi?"
        if self.is_reading:
            msg += "\n\nKoneksi serial akan ditutup otomatis."

        if not messagebox.askokcancel("Tutup Aplikasi", msg):
            return
        self.is_reading = False
