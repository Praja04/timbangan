...
Revisi v8:
  - Filler: custom chip button (canvas-based, bukan tk.Radiobutton default)
    → tampilan bersih, kotak kecil bernomor, merah saat dipilih
  - Filler dibatasi per variant:
      1–8 : Sachet YB 12,5gr PCS / RENCENG
      1–6 : Sachet YB 20gr PCS / RENCENG, Sachet BB 40gr PCS / RENCENG
      1–2 : semua Pouch
  - Filler otomatis reset & chip disabled saat variant berubah
  - Validasi ABNORMAL: berat < std/2 atau berat > std*1.5 → blok simpan
  - Konfirmasi 2-tahap SPACE → banner kuning → SPACE/klik konfirmasi
  - Serial auto-detect, reconnect manual, retry 3x
...
# Berapa filler yang boleh dipilih (1..N)
VARIANT_FILLER_MAX = {
...
def _is_abnormal(weight, std_data):
    """Berat di luar rentang std/2 .. std*1.5 dianggap abnormal."""
    if std_data is None:
        return False
    s = std_data["std"]
    return weight < s / 2.0 or weight > s * 1.5


# ════════════════════════════════════════════════════════════════
class FillerChip(tk.Frame):
    """
    Custom mini chip untuk satu nomor filler.
    Terlihat seperti kotak nomor kecil — bukan radio button default.
    """
    SIZE   = 26   # lebar & tinggi kotak (px)
    RADIUS = 4    # sudut rounded (simulasi via relief)
    ...
    def _draw(self, state):
        ...
        # Border rounded rectangle (via deux rectangles + ovals, simplified)
        r = self.RADIUS
...
class ChipButton(tk.Frame):
    """Selectable chip/tag button untuk variant."""
...
class ScaleApp:
    def __init__(self, root):
        ...
        self.sel_filler_val   = ""       # string "1"–"8" atau ""
    ...
    def _space_save(self, event=None):
        ...
        weight = (self._frozen_data["weight"] if self._frozen_data
                  else (self.current_data["weight"] if self.current_data else None))
        if weight is not None and self.sel_variant:
            if _is_abnormal(weight, VARIANT_STANDARDS.get(self.sel_variant)):
                self._alert_abnormal(weight, VARIANT_STANDARDS[self.sel_variant])
                return
    ...
    def _alert_abnormal(self, weight, std_data):
        lower = std_data["std"] / 2.0
        upper = std_data["std"] * 1.5
        msg = (
            f"DATA ABNORMAL — tidak dapat disimpan!\n\n"
            f"Berat terbaca   : {weight:.2f} g\n"
            f"Standar variant : {std_data['std']:.2f} g\n\n"
            f"Batas normal    : {lower:.2f} g  –  {upper:.2f} g\n"
            f"  (standar ÷ 2  hingga  standar × 1,5)\n\n"
    ...
    def _build_col3(self, parent):
        ...
        # Abnormal inline banner (tersembunyi)
    ...
    def _build_filler_row(self, parent, row):
        ...
        # Buat semua 8 chip, awalnya semua disabled
    ...
    def _manual_reconnect(self):
        ...
        def _do():
            ...
            if self.serial_conn:
                ...
                except Exception as e: print(f"[SERIAL] close: {e}")
            self.serial_conn = None
            time.sleep(0.8)
            self._reconnecting = False
    ...
    def _validate_display(self, weight):
        ...
        std_val = std["std"]
        lower_ab = std_val / 2.0
        upper_ab = std_val * 1.5

        if weight < lower_ab or weight > upper_ab:
            if weight <= 0:
                label = "ABNORMAL — Timbangan kosong / berat 0 atau minus"
            elif weight < lower_ab:
                label = f"ABNORMAL — Jauh di bawah standar (< {lower_ab:.1f}g)"
            else:
                label = f"ABNORMAL — Jauh di atas standar (> {upper_ab:.1f}g)"
            ...
            self.valid_lbl.config(
                text=f"Normal: {lower_ab:.1f}–{upper_ab:.1f}g  |  Std={std_val:.2f}g",
                fg=ABNORMAL_FG)
    ...
    def _show_abnormal_banner(self, weight, std_data):
        std_val = std_data["std"]
        lower = std_val / 2.0; upper = std_val * 1.5
        self.abnormal_lbl.config(
            text=f"⛔ DATA ABNORMAL — tidak dapat disimpan\n"
                 f"Batas normal: {lower:.1f}g – {upper:.1f}g  |  Berat: {weight:.2f}g")
    ...
    def _pick_filler(self, val):
        """Callback saat chip filler diklik."""
        # Deselect semua dulu
    ...
    def _update_filler_chips(self, variant_name):
        """
        Enable chip 1..N sesuai variant, disable sisanya.
        Reset pilihan filler.
        """
        self.sel_filler_val = ""
        ...
        self.filler_hint_lbl.config(
            text=f"Nomor filler (1–{max_f}):", fg=GRAY_600)
