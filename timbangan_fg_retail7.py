...
Revisi v9:
  - FIX: frozen data tidak hilang saat banner disembunyikan
    → _hide_confirm_banner() tidak lagi me-reset _frozen_data
    → frozen disimpan lokal sebelum hide, dikirim langsung ke _save_data
  - FIX: deteksi COM port prioritaskan Prolific/USB-Serial
    → port dengan deskripsi/manufacturer mengandung "prolific" atau "usb"
       dicoba PERTAMA, sisanya belakangan
    → menghindari COM1/COM3 (port virtual Windows) terpilih duluan
...
class ScaleApp:
    ...
    def _space_save(self, event=None):
        ...
        if self._confirm_pending:
            ...
            # Ambil frozen SEBELUM hide (hide tidak mereset frozen lagi)
            frozen = self._frozen_data
            if frozen is None:
                return  # safety guard
            # Cek sekali lagi (berjaga-jaga)
    ...
    def _confirm_and_save(self):
        ...
        frozen = self._frozen_data          # ambil dulu sebelum direset
        self._confirm_pending = False
        self._hide_confirm_banner()
        self._frozen_data = None            # baru reset setelah diambil
        if frozen is None:
            return
        if _is_abnormal(frozen["weight"], VARIANT_STANDARDS.get(self.sel_variant)):
            self._alert_abnormal(frozen["weight"], VARIANT_STANDARDS[self.sel_variant])
    ...
    # ─────────────────────────────────────────────────────────────
    # SAVE DATA  ← parameter baru: source_override
    # ─────────────────────────────────────────────────────────────
    def _save_data(self, source_override=None):
        """
        Simpan data timbangan.
        source_override : dict frozen yang sudah di-capture saat SPACE pertama.
                          Jika None, pakai current_data (fallback).
        """
        ...
        self._frozen_data = None          # bersihkan frozen setelah berhasil disimpan
